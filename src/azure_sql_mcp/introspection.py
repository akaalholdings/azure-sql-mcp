    ...
    async def get_table_stats(
        ...
        WITH partition_storage AS (
            ...
            SELECT
                ...
                SUM(
                    CASE
                        WHEN index_id IN (0, 1) THEN partition_rows
                        ELSE 0
                    END
                ...
                SUM(
                    CASE
                        WHEN index_id IN (0, 1) THEN used_pages
                        ELSE 0
                    END
                ) AS data_used_pages,
                SUM(
                    CASE
                        WHEN index_id > 1 THEN used_pages
                        ELSE 0
                    END
                ) AS index_used_pages
        ...
        SELECT
            ...
            CAST(
                ts.data_used_pages * 8.0 / 1024
                AS DECIMAL(18, 2)
            ) AS data_size_mb,
            CAST(
                ts.index_used_pages * 8.0 / 1024
                AS DECIMAL(18, 2)
            ) AS index_size_mb,
        ...
        INNER JOIN table_storage AS ts
            ON t.object_id = ts.object_id
        LEFT JOIN nonclustered_index_counts AS ic
